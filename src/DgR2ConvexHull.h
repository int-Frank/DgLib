...
namespace Dg
{
  namespace R2
  {
    ...
    template <typename Real, typename Container>
    void ConvexHull_Mutable(R2::Vector<Real>* a_points, size_t a_pointCount, Container& a_out)
    {
      if (a_points == nullptr)
        return;
      ...
      // Place the bottom-most point at first position 
      {
        R2::Vector<Real> temp = a_points[0];
      ...
      // has larger polar angle (in counterclockwise 
      // direction) than p1 
      std::sort(&a_points[1], &a_points[1] + a_pointCount - 1, [p0 = a_points[0]](R2::Vector<Real> const& a_p1, R2::Vector<Real> const& a_p2)
      {
        Orientation o = impl_ConvexHull::GetOrientation(p0, a_p1, a_p2);
        if (o == Orientation::Colinear)
          return p0.SquaredDistance(a_p1) <= p0.SquaredDistance(a_p2);
      ...
      for (size_t i = 1; i < a_pointCount; i++)
      {
        ...
        // with respect to p0 
        while (i < a_pointCount - 1
          && impl_ConvexHull::GetOrientation(a_points[0], a_points[i], a_points[i+1]) == Dg::Orientation::Colinear)
          i++;
